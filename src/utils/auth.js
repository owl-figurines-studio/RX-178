...
class Auth {
  ...
  static appCheckAuth() {
    return new Promise(function (resolve) {
      ...
      //如果有授权信息
      if (Auth.checkAuth() && !state.app.user.appOnLaunch) {
        //直接返回
        resolve(true);
      } else {
        //判断session_key是否过期
  ...
  static checkAuth() {
    const state = Taro.$store.getState();
    //从缓存读取授权信息
    let authorize = state.user.authorize || Taro.getStorageSync('authorize') || {},
      expiryTime = 0,
...
async function getAuthToken() {
  ...
  let response = await Taro.request({
    url: `${state.user.baseURL}api/user/authorize`,
    data: {
      code: res.code
    },
    method: 'POST'
